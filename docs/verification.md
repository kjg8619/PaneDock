...
1. **추적 콜백에 `@objc`가 없었다(고쳤지만 그것만으로는 부족했다).** `NSTrackingArea`는 owner를 ObjC 메시징으로 부르는데
   `mouseEntered(with:)`/`mouseExited(with:)`에 `@objc`가 없어 **조용히 아무 일도 일어나지 않는 상태**였다
   (프로토콜 채택이 없으면 Swift는 자동으로 `@objc`를 붙이지 않는다). `@objc`를 붙인 뒤에도
   **`mouse=entered`/`mouse=exited` 이벤트가 로그에 남지 않았다** — 즉 이탈 감지 경로는 **아직 동작하지 않는다.**
   진단 로그는 원인을 이어서 볼 수 있도록 남겨 두었다.
...
- **마우스 이탈 감지 자체가 아직 동작하지 않는다.** `NSTrackingArea`를 SwiftUI `NSHostingView` 위에 붙인 방식이
  비활성 패널에서 enter/exit를 받지 못하는 것으로 보인다(후보: hosting view의 자체 추적과의 충돌, `.inVisibleRect`, `becomesKeyOnlyIfNeeded`).
  다음 수정에서 추적 방식 자체를 바꿔야 한다.
- **손잡이 hover로 펼쳐지는 순간**(같은 이유).
- **단축키(⌃⌥⌘D) 호출과 그로 인한 키보드 실행.** 합성 단축키 이벤트가 앱에 도달하지 않았다.
  제품 결함으로 단정하지 않는다 — 메뉴(`Dock 호출/닫기`) 경로로 원인을 구분하고, 필요하면 **사람의 실제 단축키 입력**을 받아 확인한다.
