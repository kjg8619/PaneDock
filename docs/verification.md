...
### V18.15 넘침 조작 보완 (밀린 카드·전부 밀린 공통)

| 문제 | 고친 것 |
| --- | --- |
| 밀린 카드의 타이머를 **볼 수만** 있었다 | 상세 보기 `카드` 줄에 카드마다 **시작·일시정지·재설정** 버튼을 넣고, 같은 카드 id·같은 `performTimer` 경로를 쓴다. 밀린 카드도 `밀림` 표시와 함께 조작할 수 있다 |
| 공통 항목이 **전부 밀려** `display.common`이 비면 `+N` 타일에 키보드로 갈 수 없었다 | `DockFocusPlan`이 화면과 **같은 기준**(카탈로그의 공통 항목 수)으로 구성요소를 판단한다. 상세 보기가 열려 있으면 밀린 카드의 조작도 초점 목록에 들어간다 |

자체 검사 2건 추가(총 **171/171**): 공통이 전부 밀린 화면에서 첫 조작이 `overflow:common` ·
밀린 카드의 `▶❚❚↺`는 상세 보기를 열었을 때만 초점 목록에 들어간다.

**실제 GUI 관측(세션 잠금 해제 상태, 임시 설정·가짜 입력):**

| 확인 | 결과 |
| --- | --- |
| 키보드 경로(초점 목록·활성화) | 로그 `keyselftest list=[item…,timer:focus-1:start,pause,reset,…]` → `activate control=timer:focus-1:start source=keyboard` + `card=focus-1 action=start/pause/reset source=keyboard` → `overflow:cards` 활성화로 상세 보기 열림(`details=true`) |
| 밀린 카드 조작(마우스) | 상세 보기에서 `밀림` 표시가 있는 focus-3의 `시작` 클릭 → `card=focus-3 action=start source=mouse` · 화면에 `24:48 집중 중 밀림`(`shots/act-9-hidden-running.png`) |
| ⋯ 메뉴(비활성 패널에서 마우스) | `menu=open source=mouse` + 메뉴 화면(상세 보기·폴더 열기·경로 복사·고정·공간 부족 안내) 캡처(`shots/act-6-menu-mouse.png`) |
| 편집창 카드 추가·제거 | 카드 8→9 추가(새 id `focus-9`), 제거 11→10(`focus-2` 빠짐), 추가 시 **지운 id를 재사용하지 않음**(`focus-12`) |
| 저장 | `layout=save result=ok` · `editor=save result=ok saved=1 unchanged=1` → `settings.json`의 카드 목록만 바뀜(카탈로그 파일은 그대로) |

**키보드 물리 입력에 대한 한계(기록):** macOS는 다른 앱이 앞에 있을 때 **합성 키 이벤트로 이 앱을 활성화하지 못하게** 한다
(`NSApp.activate`·AX `set frontmost` 모두 거부 — frontmost가 ghostty로 유지됨). 그래서 키 입력 자체는 사용자 확인(V17.11)에
의존하고, 이번에는 **키가 들어왔을 때 타는 같은 함수**(`moveFocus`·`activateFocusedItem`)를 순서대로 실행해 초점 목록과 조작 연결을 확인했다.

### V18.16 실제 Ghostty 전환 (합성 전환과 구분)

`--fake` 없이 임시 카탈로그(홈/PaneDock/CodeMose)로 실행하고, **Ghostty 자체 UI 동작**(탭 전환 `⌘2`)으로 실제 포커스를 바꿨다.

| 단계 | 관측 |
| --- | --- |
| 시작(포커스 pane = `~/`) | `folder=kangjingoo path=/Users/kangjingoo project=p-home(6) shown=4 hidden=2 area=360 bar=1090` · 화면에 `프로젝트 · 홈`(`shots/live-1-home.png`) |
| 실제 ▶ 클릭으로 타이머 시작 | `card=focus-1 action=start source=mouse remaining=25:00` → 화면 `23:13 집중 중` |
| **실제 pane 전환**(`⌘2`, cwd `/`) | `project=- shown=3 hidden=0 bar=1090` · 화면이 **미등록 영역**(주황 실선 + `/ · 등록된 프로젝트가 없습니다` + `+`)으로 바뀜(`shots/live-2-unregistered.png`) |
| 유지 확인 | 같은 전환 동안 **타이머 23:13 → 23:08 계속**, 앱·카드 위치와 바 폭(1090) 그대로 |
| 복귀(`⌘1`) | `project=p-home(6) bar=1090`(`shots/live-3-back-home.png`) |

합성 전환(`--fake toggle/steady`)과 위 실측은 **다른 표에 기록**했다 — 합성은 항목 수가 다른 두 프로젝트를 오가며
배치 규칙을 보는 데 쓰고, 실측은 실제 Adapter 읽기·실제 포커스 변경에서도 같은 화면이 유지되는지 보는 데 썼다.
**cmux 전환은 하지 않았다**(cmux 인스턴스가 없어 실측 불가).

### V18.17 미확인·남은 것

- ~~A 기반 시안 자료 없음~~ → **확인됨**(`~/Downloads/PaneDock_Ref.png`). V18.18에서 적용했다.
- 저장 시 `card=prune` **로그 줄 자체는 관측하지 못했다**(제거·저장은 파일 내용으로 확인). 코어 검사가 같은 규칙을 고정한다.
- 편집창 카드 조작은 좌표 클릭으로 확인했다(AX 트리에 버튼이 노출되지 않는다).
- 키보드 물리 입력(⌃⌥⌘D → Tab → Enter)은 위 한계로 이번에도 사용자 확인이 필요하다.
- 실제 화면에서 **화면 잠금 중** 창 단위 캡처는 유리 재질이 합성되지 않아 배경이 어둡게 나온다(잠금 해제 후 캡처는 정상).

### V18.18 A 기반 시안 적용 (사용자 제공 `~/Downloads/PaneDock_Ref.png`)

이번에 **A 기반 시안을 새 시각 기준**으로 받았다(경로 확인). 시안 이미지를 에셋으로 붙이지 않고 형태를 코드로 그렸다.

| 적용 | 내용 | 확인 |
| --- | --- | --- |
| 앱 아이콘 | `assets/PaneDockSymbol`(Swift)이 시안의 pane 형태(둥근 사각형 + 두 pane + 점 + 아래 막대 3개)를 **파란 그라디언트 타일**로 그린다. `assets/make-icon.swift`가 16~1024px + `assets/PaneDock.icns`를 만들고, `make-app.sh`가 `Contents/Resources`에 넣고 `CFBundleIconFile`을 설정한다 | 아이콘 렌더 확인, `dist/PaneDock.app`에 `PaneDock.icns` 포함 |
| 메뉴바 심볼 | 같은 형태의 **단색 템플릿** 이미지(`isTemplate = true`)를 상태 항목에 쓴다(글자 제목 제거). 가짜 모드는 흐리게 | 메뉴 막대 캡처(`shots/act-13-menubar.png`) |
| 프로젝트 항목 | **넓은 가로 칩 → 아이콘 + 하단 짧은 이름**(64pt). `projectTileLabeledWidth = 64`(기존 칩 112) | `shots/act-12-A-project-tiles.png` — `알`/`A 문서` 타일 아래 이름 |
| 아이콘 중심 모드 | 이름을 그리지 않아 툴팁·접근성 이름만 남는다(유지) | 자체 검사 + 기존 캡처(`2-icononly-A.png`) |

**기존 HTML 시안과 다른 부분(명시):**

| 요소 | 기존 HTML(승인 V18) | A 기반 시안 | 이번 처리 |
| --- | --- | --- | --- |
| 프로젝트 항목 | 48pt 타일 / 이름은 별도 라벨 | 타일 + **하단 이름** | A 시안 적용(폭 64) |
| 메뉴바 | 글자 제목 | **pane 단색 심볼** | A 시안 적용 |
| 앱 아이콘 | 없음 | pane 형태 아이콘 | A 시안 적용(에셋+번들) |
| 프로젝트 영역 | 점선 + 좌상단 라벨 칩, `+N` 타일 | **밝은 패널 + 헤더(상태 점·이름·호스트 칩·⋯) + `+ 추가`** | **미적용** — 폭·위치 유지 규칙과 `⋯` 위치를 바꾸게 된다 |
| 미등록 상태 | 주황 실선 + `+` 타일 | 주황 점 + `폴더 열기`·`프로젝트로 등록` 버튼 | **미적용**(`폴더 열기`는 ⋯ 메뉴에 이미 있다) |
| 공통 카드 | 아날로그 시계 + 진행 링 타이머 | **큰 디지털 시간 + 날짜 + 캡션**, 링 + `Focus` | **미적용**(카드 규격 변경은 별도 판단 필요) |
...
## V20.2. 2026-09-18 — V20.1 적용·복구 안정화

V20.1의 모드 모델·메뉴·동의 창·시스템 제어·복구 명령은 그대로 두고 **경로의 결함**을 고쳤다.
실제 시스템 Dock 설정 변경은 **여전히 하지 않았다**(승인 대기).

### V20.2.1 고친 결함

| 결함 | 고친 방식 | 검사 |
| --- | --- | --- |
| `set`이 값을 바꾼 뒤 오류를 던지면 그 키가 되돌리기에서 빠졌다 | 쓰기 **시도 전에** 되돌릴 목록에 넣고, 쓰기 오류 뒤에도 **실제 값을 확인**해 판정 | `값을 바꾼 뒤 오류를 던진 키도 되돌리기 대상에 포함된다` |
| 되돌리기가 반환값만 보고 성공으로 봤다 | 값 확인 실패는 실패로 보고, 기록에 `rollbackFailed`로 남긴다 | `되돌림이 확인되지 않으면 기록을 남기고 실패로 보고한다` |
| 복원이 '사용자 변경'과 '이미 원래 값'·'키 삭제'·'실패'를 구분하지 않았다 | 처분 5종으로 구분하고 결과 종류(완전/부분/실패/없음)와 함께 보고 | `사용자가 실행 중 바꾼 값은 …`, `이미 원래 값인 키는 …`, `원래 있던 키가 사라져 있으면 …` |
| 손상된 기록을 '없음'으로 보고 새 적용이 덮어썼다 | 읽기 결과를 없음/읽음/읽을 수 없음으로 구분, 손상 시 **적용도 거부**하고 원본 보존 | `손상된 복구 기록이 있으면 새 적용도 거부하고 원본을 보존한다` |
| 기록 저장·삭제 실패가 `Bool`/`try?`로 삼켜졌다 | 쓰기·갱신·삭제 결과를 종류로 돌려주고 결과·로그·화면에 남긴다 | `기록 저장에 실패하면 적용하지 않는다`, `기록 삭제 실패를 삼키지 않고 …` |
| 같은 작업의 진행 갱신이 '미복원 기록'으로 막혔다 | `operationID`로 새 작업과 진행 갱신을 구분 | `같은 작업의 진행 갱신은 되고, 다른 작업 ID로는 갱신되지 않는다` |
| 잠금이 파일 존재 기반이라 죽은 PID의 잠금이 복구를 막았다 | 생존 + 부팅 세션 + 리스로 stale 판정, 이어받을 때 원본을 사본으로 보존, 살아 있는 소유자는 건드리지 않음 | `죽은 프로세스·지난 리스의 잠금은 근거를 남기고 이어받는다`, `살아 있는 다른 인스턴스의 잠금은 지우지 않고 거부한다` |
| `.double(1000)`이 정수로 읽혀 자료형을 잃었다 | `CFNumberIsFloatType`으로 부동소수를 구분해 읽고(`fromNumber`), 비교는 수치로 통일 | `설정값: 정수/실수가 바뀌어 읽혀도 …`, `설정값: 시스템이 정수로 돌려줘도 …` |
| 준비가 끝나기 전에 시스템 설정을 썼다 | `prepareScreen`이 **하단 배치·표시·확인**까지 끝낸 뒤에만 적용 단계로 간다(순서를 검사로 고정) | `화면 준비가 끝난 뒤에야 설정을 쓴다` |
| 크기가 같으면 하단 배치가 생략됐다 | Custom 진입은 `force`로 항상 배치를 확정하고 `anchor=bottom … moved=` 로그를 남긴다 | 승인 후 실행에서 확인(로그) |
| Mac Dock 복귀 시 패널·입력 세션이 남았다 | 복원 경로가 `endInvocation`(키 모니터·키보드 선택 해제) + 자유 좌표 복귀 + 창 닫기를 함께 한다 | 승인 후 실행에서 확인 |
| 자유 좌표와 하단 배치가 섞여 저장됐다 | Custom 중 드래그는 저장하지 않고 하단으로 되돌리며, `freeOrigin`은 따로 보존 | 승인 후 실행에서 확인 |
| 부분 복원을 "복원했습니다"로 알렸다 | 결과 종류별 문구(되돌릴 것 없음/완전/부분/실패)를 쓰고 부분은 남은 항목을 함께 보여준다 | `실제 복원 실패를 '실패'로 구분하고 부분 복원을 '복원했습니다'로 알리지 않는다` |
| 지난 적용 실패 뒤 다음 실행이 같은 실패를 반복했다 | 실패 시각을 남기고 다음 실행은 자동 적용하지 않는다(성공하면 지운다) | 코드 경로 + 시작 로그 `startup apply=skipped lastFailure=` |

### V20.2.2 자체 검사 — 새 16건 포함 **213/213** (실행 결과)

```
PASS  모드: 동의 없이는 시스템 설정을 바꾸지 않는다  — 거부=true 쓰기=0 재시작=0
PASS  모드: 적용은 바꾸는 키만 기록하고 이미 원하는 값인 키는 건드리지 않는다  — 기록=autohide 쓰기=set autohide=true
PASS  모드: 재등장 억제 설정은 승인했을 때만 적용한다  — 억제키=1000.0 기록=3개
PASS  모드: 화면 준비가 끝난 뒤에야 설정을 쓴다  — 순서=prepare>set:autohide>set:autohide-delay>set:autohide-time-modifier>restart
PASS  모드: 화면 준비 실패는 시스템 설정을 바꾸지 않고 기록도 남기지 않는다  — 실패=true 쓰기=0 기록=없음
PASS  모드: 적용 실패 시 이미 바꾼 키를 값 확인까지 마치고 되돌린다  — 되돌림=false 기록=정리됨 재시작=0
PASS  모드: 값을 바꾼 뒤 오류를 던진 키도 되돌리기 대상에 포함된다  — 값=false 기록=정리됨 writes=2
PASS  모드: 되돌림이 확인되지 않으면 기록을 남기고 실패로 보고한다  — 실패=true 남은키=test.panedock.dock:autohide
PASS  모드: 복구 정보를 기록하지 못하면 시스템 설정을 바꾸지 않는다  — 거부=true 쓰기=0
PASS  모드: 기록 저장에 실패하면 적용하지 않는다(실패를 삼키지 않는다)  — 거부=true 쓰기=0
PASS  설정값: 정수/실수가 바뀌어 읽혀도 같은 값으로 보고, 원래 자료형은 따로 남긴다  — double(1000)읽기=double int(1000)읽기=int
PASS  설정값: 시스템이 정수로 돌려줘도 적용 확인이 통과하고 복원은 원래 자료형으로 되돌린다  — 적용기록=1000.0/double 복원=complete 원래값=30.0/double
PASS  모드: 복원은 원래 값만 되돌리고 없던 키를 지우며 반복해도 안전하다  — 복원=3키 분류=complete 반복=nothingToDo 재시작=2
PASS  모드: 사용자가 실행 중 바꾼 값은 덮어쓰지 않고 '사용자 변경'으로 구분한다  — 분류=partial 사용자변경=1 값=7.0 기록=1개
PASS  모드: 이미 원래 값인 키는 '이미 원래 값'으로 구분하고 기록에서 뺀다  — 분류=complete 이미원복=1 기록=정리됨
PASS  모드: 원래 있던 키가 사라져 있으면 '키 삭제됨'으로 구분하고 만들지 않는다  — 분류=partial 삭제=1 값=없음
PASS  모드: 실제 복원 실패를 '실패'로 구분하고 부분 복원을 '복원했습니다'로 알리지 않는다  — 분류=partial 실패=1 문구=일부만 복원했습니다 — 복원 2키 · 실패 1키 · 기록을 남겨 두었습니다.
PASS  모드: 재시작 실패는 기록에 남기고 다음 복원에서 재시작만 다시 한다  — 1차=partial 대기기록=true 2차=complete 재시작=2
PASS  모드: 기록 삭제 실패를 삼키지 않고 '기록이 남아 있다'고 보고한다  — 기록남음=true 파일=true 안내=복구 기록 정리 실패: “dock-recovery.json” couldn’t be removed because you don’t have permission to access it.
PASS  모드: 다른 작업의 미복원 기록이 있으면 새 적용도 거부하고 원본을 보존한다  — 새기록=refusedPendingRestore(["test.panedock.dock:autohide"]) 적용거부=true 보존=other-operation
PASS  모드: 같은 작업의 진행 갱신은 되고, 다른 작업 ID로는 갱신되지 않는다  — 같은작업=written 다른작업=refusedPendingRestore(["test.panedock.dock:autohide"])
PASS  모드: 손상된 복구 기록이 있으면 새 적용도 거부하고 원본을 보존한다  — 거부=true 보존=not json 쓰기=0
PASS  모드: 손상된 기록으로는 복원하지 않고 실패로 보고하며 파일을 지우지 않는다  — 오류=복구 기록 파일을 읽을 수 없습니다(형식이 맞지 않습니다) — 지우지 않고 남겨 두었습니다. 파일을 확인한 뒤 직접 되돌려 주세요. 파일있음=true
PASS  모드: 살아 있는 다른 인스턴스의 잠금은 지우지 않고 작업을 거부한다  — 거부=true 잠금주인=live-other/9999
PASS  모드: 죽은 프로세스·지난 리스의 잠금은 근거를 남기고 이어받는다  — 이어받음=true 사본=1개 새주인=recovery
PASS  모드: 잠금 stale 판정은 근거로만 하고 살아 있는 최근 잠금은 건드리지 않는다  — 리스초과=작업 시각이 2000초 지남(리스 600초 초과) 다른부팅=다른 부팅 세션의 잠금 살아있음=stale 아님
PASS  모드: 비정상 종료 뒤 다음 실행에서 복구 상태를 감지한다(자동 복원 아님)  — 감지=true 적용상태=적용 안 됨(기본 Dock 사용)
PASS  모드: Both는 미구현으로 표시되고 적용 대상이 아니다  — both=false 목록=macDock,custom,both
PASS  복구 기록: 저장·복원 왕복이 되고, 없는 파일은 '없음'으로 구분한다  — 왕복=true 읽기=미복원 기록 1키
```

검사는 전부 **대체 시스템 제어**(가짜 Dock)와 임시 파일만 쓴다. Dock 재시작도 대체 구현이 맡는다.

### V20.2.3 실제 경로 관측 (읽기 전용 · GUI)

| 관측 | 결과 |
| --- | --- |
| 실제 `com.apple.dock`의 부동소수 키 | `--dock-probe largesize` → `76.0/double`, `defaults read-type` → `Type is float` — 값이 정수처럼 보여도 **자료형을 보존**해 읽는다 |
| 계획 키 현재 값 | `autohide=false/bool`, `autohide-delay=없음`, `autohide-time-modifier=없음` (되돌릴 때 두 키는 **삭제** 대상) |
| 시작 감지(미복원 기록 1키) | 로그 `dockmode stale detected keys=1` |
| 메뉴(복원 진입점) | `복구 기록: 미복원 기록 1키` 표시, `Custom Dock 적용 (미복원 기록을 먼저 복원)` 비활성, **`기본 Dock으로 복원` 활성** — Custom이 아닌 상태에서도 복구에 접근 가능. 캡처 `/tmp/v18c/m3s.png` |
| CLI 종료 코드 | 기록 없음 → `nothingToDo` **0** · 사용자 변경/키 삭제가 남은 기록 → `partial … ` **2** · 손상 기록 → `failed reason=unreadable-record(…) file=…` **3** (파일 보존 확인) |
| 운영 Dock 무영향 | 위 실행 모두 `--no-dock-restart` · `com.apple.dock` 내용 md5 동일 · 재시작 로그 `restart=false` |

### V20.2.4 승인 후에만 하는 것

`Mac Dock → Custom(적용) → 하단 사용 → Mac Dock 복귀 → 정상 종료 복원 → 미복원 상태 복구` 흐름과
그 상태의 승인 화면 확인은 **사용자 승인 뒤에** 실행한다(V20.1.6의 변경 항목과 같다).

### V20.2.5 창 연결(하단 배치·준비 확인·자유 좌표) — 미리보기에서 확인

`--preview-custom`은 **시스템 설정을 바꾸지 않지만 준비 경로는 실제와 같은 함수**를 지난다.
아래는 그 실행에서 나온 로그와 창 좌표다(적용 성공으로 기록하지 않는다).

| 확인 | 관측 |
| --- | --- |
| 진입 시 하단 배치(크기가 같아도) | `anchor=bottom origin=(186,90) moved=true freeOrigin=(200,520)` |
| 같은 자리일 때도 배치를 다시 적용 | `anchor=bottom origin=(186,90) moved=false freeOrigin=(200,520)` |
| 준비 완료 확인(시스템 설정을 쓰기 전) | `prepare=ready:true visible=true origin=(186,90) screenBottom=90` |
| 자유 좌표 보존 | `corrected-origin skipped (하단 기준)` · 설정 파일 `windowOrigin`이 `(200,520)` 그대로 |
| 드래그 무시 + 하단 복귀 | `drag ignored (Custom: 하단 기준)` → `anchor=bottom … moved=true`, 창 `186,788 1140×104`(하단 892 = 화면 982 − Dock 90) |

`--move-after <ms>`(진단용)를 추가해 사용자 드래그와 같은 `windowDidMove` 경로를 시간차로 지나가게 했다.

아직 실행하지 않은 것(승인 후): 실제 적용 상태에서의 **복귀 정리**(`endCustomSession` — 키 모니터 해제·자유 좌표 복귀·창 닫기),
시스템 Dock이 실제로 숨겨진 화면, 그리고 정상 종료 복원·미복원 복구 흐름.

## V20.3. 2026-09-18 — V20.1 시작·복구 경계 완결

이번에도 **실제 시스템 Dock 설정 변경은 하지 않았다**(승인 대기). 아래는 "구현한 보호 조건"과
"실제로 수행한 검증"을 구분해 적는다.

### V20.3.1 구현한 보호 조건

| 영역 | 보호 조건 |
| --- | --- |
| 시작 | 컨트롤러 초기화(`installDockModeController`)와 저장된 모드 적용(`applySavedModeAtStartup`)을 분리. 적용은 **패널·메뉴 준비 뒤** 실행. 결정은 `DockStartupPolicy`(미완료 복구 → 선택 없음 → 동의 → 지난 실패 순) |
| 화면 | Custom 기준 위치를 **대상 화면의 실제 아래 가장자리**로 계산(`ScreenGeometry.bottomAnchorOrigin`). 자유 좌표는 계속 `visibleFrame` 기준 |
| 복원 미완료 | `적용 상태: 복원 미완료(남은 항목 N)` + `복원 재시도 가능` + `사용자가 바꾼 항목 정리` 메뉴. 종료 복원은 **우리가 적용한 세션만** |
| 기록 | 필수(`mode`·`appliedAt`·`entries`) 손상 → '읽을 수 없음', 이전 형식 선택 필드 누락 → 기본값. 항목 목록을 빈 배열로 대체하지 않음. 항목 0개+`restartPending`도 미완료. 바꿀 설정 0개여도 기존 기록 먼저 확인. `recordLeft`는 실제 파일 재확인 |
| 사용자 변경 | 이후 복원에서 값 재확인(되돌릴 수 있으면 되돌림), 자동 삭제 없음, 명시 정리는 GUI 메뉴/`--forget-user-changed` |
| 잠금 | `flock`(fd) 배타 잠금. 시간 경과로 빼앗지 않음. 죽은 프로세스는 OS가 풀어 줌(남은 내용은 사본 보존). 복구 읽기·판정·변경은 잠금 안에서 |

### V20.3.2 자체 검사 — 새 9건 포함 **219/219** (실행 결과)

검사는 전부 **대체 시스템 제어**(가짜 Dock)와 **임시 파일**에서 수행하고, Dock 재시작도 대체 구현이 맡는다.

```
PASS  잠금: 같은 파일에 동시에 하나만 잡힌다(두 번째는 거부)  — A=acquired B=heldByOther 내용=op-a 풀린뒤B=acquired
PASS  잠금: 살아 있는 소유자의 잠금은 시간이 지나도 빼앗지 않는다  — 잡음=acquired 재시도=heldByOther 주인=long-lived
PASS  잠금: 비정상 종료 흔적은 이어받고 원본을 사본으로 남긴다  — 이어받음=acquired 흔적=crashed 사본=1개
PASS  잠금: 다른 인스턴스가 잡고 있으면 복구도 거부하고 기록을 그대로 둔다  — 거부=true 기록전=1 기록후=1
PASS  시작: 저장된 모드 적용은 컨트롤러 초기화와 분리된 결정 함수가 정한다  — 적용=true 미완료=미완료 복구 기록 1건 재시작만=미완료 복구 기록 1건 지난실패=지난 적용 실패(2026-09-18T01:00:00Z) — 자동으로 다시 시도하지 않습니다
PASS  기록: 필수 정보(항목 목록) 손상은 '읽을 수 없음'으로, 이전 형식의 선택 필드 누락은 기본값으로 읽는다  — 항목없음=복구 기록을 읽을 수 없음(형식이 맞지 않습니다) 항목손상=복구 기록을 읽을 수 없음(형식이 맞지 않습니다) 이전형식=미완료 기록(미복원 1키)
PASS  기록: 바꿀 설정이 0개여도 미완료·손상 기록을 먼저 확인하고 거부한다  — 미완료거부=true 보존=op-pending 손상거부=true
PASS  기록: 항목 0개 + 재시작 대기도 미완료 복구로 보고 재시작을 마친 뒤에만 정리한다  — 생성=written 새작업=refusedPendingRestore(["Dock 재시작 대기"]) 결과=complete 재시작=true 기록남음=false
PASS  기록: 사용자 변경 항목은 이후 상태를 다시 보고, 명시적으로 정리할 때만 기록에서 뺀다  — 1차=partial 유지=userChanged 2차=complete 2차값=30.0 정리전=1 정리=written 정리후=0 값=9.0
```

앞선 V20.2 검사 30여 건(준비 순서·쓰기 뒤 오류·되돌림 값 확인·기록 실패·손상 보존·결과 4분류·자료형)도 함께 통과한다.

### V20.3.3 실제 수행한 관측 (GUI·CLI, 시스템 변경 없음)

| 관측 | 결과 |
| --- | --- |
| 시작 순서(재실행 경로) | 로그 순서가 `controller=ready` → **`ready panel=built(1090.0x104.0) menu=installed`** → `startup decision=…` — 창·메뉴 준비 뒤에 결정·적용이 온다 |
| 시작 결정(저장된 Custom + 동의 + 미완료 기록) | `startup decision=recoverFirst reason=미완료 복구 기록 1건` → `startup recovery-first`. **자동 적용하지 않았고** 커스텀 패널도 뜨지 않았다. `com.apple.dock` md5 동일 |
| 시작 결정(저장된 Custom + 동의 + 지난 실패) | `startup decision=skipAfterFailure reason=지난 적용 실패(2026-09-18T07:00:00Z)` → `startup apply=skipped`. 같은 실패를 자동으로 반복하지 않는다. `com.apple.dock` md5 동일 |
| 복원 미완료 구분 | 메뉴: `적용 상태: 복원 미완료(남은 항목 2)` · `복원 재시도 가능(남은 항목 2)` · `복구 기록: 미복원 2키` · `기본 Dock으로 복원`(활성) · `사용자가 바꾼 항목 1개 기록에서 정리`. 캡처 `/tmp/v18c/mp2s.png` |
| 사용자 변경 항목 정리 | 그 항목을 클릭 → 로그 `dockmode forget result=ok` · `forget-user-changed result=ok`, 기록은 `[other]` 1건만 남고 **테스트 도메인 값은 그대로**(쓰지 않음) |
| 정상 종료 가드 | 우리가 적용한 세션이 아닌 상태에서 메뉴 › 종료 → 로그 `dockmode quit-restore skipped (다른 인스턴스 세션일 수 있음)`, 복구 기록 유지 |
| Custom 기준 위치(미리보기) | `anchor=bottom origin=(186,0)` · `prepare=ready:true screenBottom=0` — 창 아래 끝이 화면 끝(982)에 맞는다. 이전 기준(`visibleFrame` = 90)과 다르다 |
| 운영 Dock 무영향 | 모든 실행 `--no-dock-restart`, `com.apple.dock` 내용 md5 세션 전후 동일(`3b76a1f7…`) |

### V20.3.4 승인 후에 실행할 것

메뉴 적용 → 실제 하단 배치 → 기본 Dock 복귀 → **저장된 Custom으로 재실행** → 정상 종료 복원.
V20.1.6의 변경 항목(autohide=true · 선택적 억제 키 2개 · Dock 1회 재시작)과 같고, 복구 방법도 같다.

## V20.4. 2026-09-18 — Custom 세션 소유권과 복구 중단 처리

실제 시스템 Dock 설정 변경은 하지 않았다(승인 대기).

### V20.4.1 구현한 보호 조건

| 영역 | 보호 조건 |
| --- | --- |
| 세션 소유권 | Custom 적용 후 **세션 동안 잠금을 계속 보유**(`sessionHandle`). 다른 인스턴스·복구 CLI는 적용·복원 모두 거부(`otherInstanceActive`). 소유자는 자기 잠금을 재사용해 해제하고, 프로세스가 죽으면 OS가 풀어 다음 실행이 복구 |
| 복구 중단 | 첫 값 쓰기 **전에** `restartPending=true` 기록 → 값 복원 뒤 재시작 전에 중단돼도 다음 실행이 재시작을 이어받는다. 값이 이미 원래 값이어도 전체 완료로 보지 않는다 |
| 되돌리기 | 적용 실패의 되돌리기도 **값 확인 + 반영(재시작)** 기준. 반영이 안 됐으면 0항목 + `restartPending`으로 기록에 남고 다음 실행이 재시작만 이어받는다 |
| 실패 후 화면·종료 | 완전 복원만 `.none` + 패널 정리. 부분·실패는 **세션 상태 유지·패널 유지**, 메뉴가 이유별로 표시. 정상 종료에서도 완전 복원이 아니면 기록 유지 |
| 검증 구분 | 같은 프로세스 객체 검사와 **별도 프로세스** 검증을 분리(자식 프로세스가 실제로 잠금을 잡고, SIGKILL 뒤 복구 가능 확인) |

### V20.4.2 자체 검사 — 새 6건 포함 **224/224** (실행 결과)

검사는 대체 시스템 제어와 임시 파일에서만 수행하고, 재시작도 대체 구현이 맡는다.

```
PASS  모드: 적용 실패 시 이미 바꾼 키를 값 확인·반영까지 마치고 되돌린다  — 되돌림=false 기록=정리됨 재시작=1
PASS  세션: 적용한 인스턴스가 계속 소유하고, 다른 인스턴스는 그 세션을 되돌릴 수 없다  — 소유=true 잠금목적=apply 다른적용거부=true 다른복원거부=true 소유자해제=complete 해제후기록=없음
PASS  잠금(별도 프로세스): 다른 프로세스가 잡으면 거부되고, 그 프로세스가 죽으면 복구할 수 있다  — 자식=실행됨 잡힌동안=heldByOther 강제종료뒤=acquired
PASS  복구 중단: 값 복원 도중 중단돼도 다음 실행이 남은 키와 반영을 이어받는다  — 중단결과=failed 중단직후값=true 남은키=1 이어받기=complete 재시작=true 값=false
PASS  복구 중단: 일부만 되돌린 상태도 다음 실행이 남은 키와 반영을 이어받는다  — 중단결과=partial 중단직후 hide=false 남은키=["autohide-delay"] 이어받기=complete
PASS  복구 중단: 적용 실패의 되돌리기도 반영이 안 됐으면 기록으로 남아 다음 실행이 이어받는다  — 적용실패=true 되돌림값=false 대기기록=true 이어받기=complete
PASS  복구 진행 기록: 첫 값 쓰기 시점에 이미 재시작 대기가 기록돼 있다  — 첫쓰기시점 restartPending=true 항목=1
```

### V20.4.3 실제 수행한 관측 (시스템 변경 없음)

| 관측 | 결과 |
| --- | --- |
| 세션 소유권(별도 프로세스) | 다른 프로세스가 세션 잠금을 잡은 동안 복구 CLI: `dock restore: failed reason=다른 PaneDock 인스턴스가 사용 모드를 적용·복원 중입니다. 다른 인스턴스(pid=71161)가 session 중` · **종료 코드 3** · 기록 그대로. 잠금이 풀린 뒤 같은 명령: `partial …` · 종료 코드 2 |
| 부분 복원 후 화면 | 메뉴 › 기본 Dock으로 복원 → 로그 `kind=partial restored=1 userChanged=1 left=true` → `dockmode restore incomplete … — 세션 유지`. 창 1090×104 유지(닫지 않음), 테스트 도메인은 `restorable=false`로 복원·`userkey=7` 그대로 |
| 메뉴 표시 | `적용 상태: 복원 미완료(사용자 변경 1 · 남은 키 1)` · `복원 재시도 가능(남은 항목 2)` · `복구 기록: 미완료 기록(미복원 2키)` · `사용자가 바꾼 항목 1개 기록에서 정리` |
| 운영 Dock 무영향 | 모든 실행 `--no-dock-restart`(로그 `dockctl restart=skipped (검증용 플래그)`), `com.apple.dock` 내용 md5 동일(`3b76a1f7…`) |
| 검증 표시 주의 | `--no-dock-restart`에서는 재시작을 건너뛰므로 결과의 `restart=true`는 "재시작 단계까지 갔다"는 뜻이다(실제 재시작 없음). |
